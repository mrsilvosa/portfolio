/*
Library:					SPI LCD - ILI9341
Written by:				Mohamed Yaqoob (MYaqoobEmbedded YouTube Channel)
Date Written:			09/12/2018
Last modified:		-/-
Description:			This is an STM32 device driver library for the ILI9341 SPI LCD display, using STM HAL libraries
										
* Copyright (C) 2018 - M. Yaqoob
   This is a free software under the GNU license, you can redistribute it and/or modify it under the terms
   of the GNU General Public Licenseversion 3 as published by the Free Software Foundation.
	
   This software library is shared with puplic for educational purposes, without WARRANTY and Author is not liable for any damages caused directly
   or indirectly by this software, read more about this on the GNU General Public License.
*/

//Header files
#include "MY_ILI9341.h"
#include "ILI9341_BSP_F4xx.h"


// legacy compatibility:

static uint8_t rotationNum=1;
static bool _cp437    = false;
static SPI_HandleTypeDef* lcdSPIhandle;
//Chip Select pin
static GPIO_TypeDef  *tftCS_GPIO;
static uint16_t tftCS_PIN;
//Data Command pin
static GPIO_TypeDef  *tftDC_GPIO;
static uint16_t tftDC_PIN;
//Reset pin
static GPIO_TypeDef  *tftRESET_GPIO;
static uint16_t tftRESET_PIN;

static //Text simple font array (You can your own font)
const unsigned char font1[] = {
	  
};

//***** Functions prototypes *****//

//1. Write Command to LCD
void ILI9341_SendCommand(uint8_t com)
{
	//*(__IO uint8_t *)(0x60000000) = com;
	uint8_t tmpCmd = com;
	ILI9341_BSP_DCset_commandMode();
	ILI9341_BSP_select_CS();
	ILI9341_BSP_SPI_transmit(&tmpCmd, 1);
	ILI9341_BSP_deselect_CS();
}

void ILI9341_SendCommand_uint16(uint16_t com)
{
	//*(__IO uint8_t *)(0x60000000) = com;
	uint16_t tmpCmd = com;
	//Set DC HIGH for COMMAND mode
	HAL_GPIO_WritePin(tftDC_GPIO, tftDC_PIN, GPIO_PIN_RESET);
	//Put CS LOW
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit(lcdSPIhandle, &tmpCmd, 1, 5);
	//Bring CS HIGH
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
}

//2. Write data to LCD
void ILI9341_SendData(uint8_t data)
{
	//*(__IO uint8_t *)(0x60040000) = data;
	uint8_t tmpCmd = data;
	ILI9341_BSP_DCset_dataMode();
	ILI9341_BSP_select_CS();
	ILI9341_BSP_SPI_transmit(&tmpCmd, 1);
	ILI9341_BSP_deselect_CS();
}

void ILI9341_SendData_uint16(uint16_t data)
{
	//*(__IO uint8_t *)(0x60040000) = data;
	uint16_t tmpCmd = data;
	//Set DC LOW for DATA mode
	HAL_GPIO_WritePin(tftDC_GPIO, tftDC_PIN, GPIO_PIN_SET);
	//Put CS LOW
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit(lcdSPIhandle, &tmpCmd, 1, 5);
	//Bring CS HIGH
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
}
//2.2 Write multiple/DMA
void ILI9341_SendData_Multi(uint16_t Colordata, uint32_t size)
{
	//uint8_t colorL,colorH;
	
	//Set DC LOW for DATA mode
	HAL_GPIO_WritePin(tftDC_GPIO, tftDC_PIN, GPIO_PIN_SET);
	//Put CS LOW
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit(lcdSPIhandle, (uint8_t *)&Colordata, size, 2000);
	//Wait for end of DMA transfer
	//Bring CS HIGH
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
}

void ILI9341_SendData_Multi_uint16(uint16_t Colordata, uint32_t size)
{
	//Set DC LOW for DATA mode
	HAL_GPIO_WritePin(tftDC_GPIO, tftDC_PIN, GPIO_PIN_SET);
	//Put CS LOW
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit(lcdSPIhandle, &Colordata, size, 2000);
	//Wait for end of DMA transfer
	//Bring CS HIGH
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
}

//3. Set cursor position
void ILI9341_SetCursorPosition(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {

  ILI9341_SendCommand (ILI9341_COLUMN_ADDR);
  ILI9341_SendData(x1>>8);
  ILI9341_SendData(x1 & 0xFF);
  ILI9341_SendData(x2>>8);
  ILI9341_SendData(x2 & 0xFF);

  ILI9341_SendCommand (ILI9341_PAGE_ADDR);
  ILI9341_SendData(y1>>8);
  ILI9341_SendData(y1 & 0xFF);
  ILI9341_SendData(y2>>8);
  ILI9341_SendData(y2 & 0xFF);
  ILI9341_SendCommand (ILI9341_GRAM);
}

void ILI9341_SetCursorPosition_uint16(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {

  ILI9341_SendCommand_uint16 (ILI9341_COLUMN_ADDR <<8);
  ILI9341_SendData_uint16(x1);
  ILI9341_SendData_uint16(x2);

  ILI9341_SendCommand_uint16 (ILI9341_PAGE_ADDR <<8);
  ILI9341_SendData_uint16(y1);
  ILI9341_SendData_uint16(y2);
  ILI9341_SendCommand_uint16 (ILI9341_GRAM <<8);
}
//4. Initialise function
void ILI9341_Init(SPI_HandleTypeDef *spiLcdHandle, GPIO_TypeDef *csPORT, uint16_t csPIN, GPIO_TypeDef *dcPORT, uint16_t dcPIN, GPIO_TypeDef *resetPORT, uint16_t resetPIN)
 {
	 //Copy SPI settings
	 //memcpy(&lcdSPIhandle, spiLcdHandle, sizeof(*spiLcdHandle));
	lcdSPIhandle = spiLcdHandle;
	 //CS pin
	tftCS_GPIO = csPORT;
	tftCS_PIN = csPIN;
	 //DC pin
	tftDC_GPIO = dcPORT;
	tftDC_PIN = dcPIN;
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
	 //RESET pin
	tftRESET_GPIO = resetPORT;
	tftRESET_PIN = resetPIN;
	HAL_GPIO_WritePin(resetPORT, resetPIN, GPIO_PIN_SET);  //Turn LCD ON

	if(lcdSPIhandle->Init.DataSize == SPI_DATASIZE_16BIT)
	{
		ILI9341_SendCommand_uint16(ILI9341_RESET <<8); // software reset comand
		HAL_Delay(100);
		ILI9341_SendCommand_uint16 (ILI9341_DISPLAY_OFF <<8); // display off
		   //------------power control------------------------------
		   ILI9341_SendCommand_uint16 (ILI9341_POWER1 <<8); // power control
		   ILI9341_SendData_uint16  (0x26 <<8); // GVDD = 4.75v
		   ILI9341_SendCommand_uint16 (ILI9341_POWER2 <<8); // power control
		   ILI9341_SendData_uint16   (0x11 <<8); // AVDD=VCIx2, VGH=VCIx7, VGL=-VCIx3
		   //--------------VCOM-------------------------------------
		   ILI9341_SendCommand_uint16 (ILI9341_VCOM1 <<8); // vcom control
		   ILI9341_SendData_uint16   ((0x35 <<8 ) | 0x3e); // Set the VCOMH voltage (0x35 = 4.025v)
		   //ILI9341_SendData_uint16   (0x3e); // Set the VCOML voltage (0x3E = -0.950v)
		   ILI9341_SendCommand_uint16 (ILI9341_VCOM2 <<8); // vcom control
		   ILI9341_SendData_uint16   (0xbe <<8);

		   //------------memory access control------------------------
		   ILI9341_SendCommand_uint16 (ILI9341_MAC <<8); // memory access control
		   ILI9341_SendData_uint16(0x48 <<8);

		   ILI9341_SendCommand_uint16 (ILI9341_PIXEL_FORMAT <<8); // pixel format set
		   ILI9341_SendData_uint16   (0x55 <<8); // 16bit /pixel

		   ILI9341_SendCommand_uint16(ILI9341_FRC <<8);
		   ILI9341_SendData_uint16((0<<8) | 0x1F);
		   //ILI9341_SendData(0x1F);
		   //-------------ddram ----------------------------
		   ILI9341_SendCommand_uint16 (ILI9341_COLUMN_ADDR <<8); // column set
		   ILI9341_SendData_uint16   ((0x00 <<8) | 0x00); // x0_HIGH---0
		   //ILI9341_SendData   (0x00); // x0_LOW----0
		   ILI9341_SendData_uint16   ((0x00 <<8) | 0xEF); // x1_HIGH---240
		   //ILI9341_SendData   (0xEF); // x1_LOW----240
		   ILI9341_SendCommand_uint16 (ILI9341_PAGE_ADDR <<8); // page address set
		   ILI9341_SendData_uint16   ((0x00 <<8) | 0x00); // y0_HIGH---0
		   //ILI9341_SendData   (0x00); // y0_LOW----0
		   ILI9341_SendData_uint16   ((0x01 <<8) | 0x3F); // y1_HIGH---320
		   //ILI9341_SendData   (0x3F); // y1_LOW----320

		   ILI9341_SendCommand_uint16 (ILI9341_TEARING_OFF <<8); // tearing effect off
		   //LCD_write_cmd(ILI9341_TEARING_ON); // tearing effect on
		   //LCD_write_cmd(ILI9341_DISPLAY_INVERSION); // display inversion
		   ILI9341_SendCommand_uint16 (ILI9341_Entry_Mode_Set <<8); // entry mode set
		   // Deep Standby Mode: OFF
		   // Set the output level of gate driver G1-G320: Normal display
		   // Low voltage detection: Disable
		   ILI9341_SendData_uint16   ((0x07 <<8));
		   //-----------------display------------------------
		   ILI9341_SendCommand_uint16 (ILI9341_DFC <<8); // display function control
		   //Set the scan mode in non-display area
		   //Determine source/VCOM output in a non-display area in the partial display mode
		   ILI9341_SendData_uint16   ((0x0a <<8) | 0x82);
		   //Select whether the liquid crystal type is normally white type or normally black type
		   //Sets the direction of scan by the gate driver in the range determined by SCN and NL
		   //Select the shift direction of outputs from the source driver
		   //Sets the gate driver pin arrangement in combination with the GS bit to select the optimal scan mode for the module
		   //Specify the scan cycle interval of gate driver in non-display area when PTG to select interval scan
		   ////ILI9341_SendData   (0x82);
		   // Sets the number of lines to drive the LCD at an interval of 8 lines
		   ILI9341_SendData_uint16   ((0x27 <<8) | 0x00);
		   //ILI9341_SendData   (0x00); // clock divisor

		   ILI9341_SendCommand_uint16 (ILI9341_SLEEP_OUT <<8); // sleep out
		   HAL_Delay(100);
		   ILI9341_SendCommand_uint16 (ILI9341_DISPLAY_ON <<8); // display on
		   HAL_Delay(100);
		   ILI9341_SendCommand_uint16 (ILI9341_GRAM <<8); // memory write
		   HAL_Delay(5);
	 }
	 else
	 {
		 ILI9341_SendCommand (ILI9341_RESET); // software reset comand
		   HAL_Delay(100);

		   ILI9341_SendCommand (ILI9341_DISPLAY_OFF); // display off
		   //------------power control------------------------------
		   ILI9341_SendCommand (ILI9341_POWER1); // power control
		   ILI9341_SendData   (0x26); // GVDD = 4.75v
		   ILI9341_SendCommand (ILI9341_POWER2); // power control
		   ILI9341_SendData   (0x11); // AVDD=VCIx2, VGH=VCIx7, VGL=-VCIx3
		   //--------------VCOM-------------------------------------
		   ILI9341_SendCommand (ILI9341_VCOM1); // vcom control
		   ILI9341_SendData   (0x35); // Set the VCOMH voltage (0x35 = 4.025v)
		   ILI9341_SendData   (0x3e); // Set the VCOML voltage (0x3E = -0.950v)
		   ILI9341_SendCommand (ILI9341_VCOM2); // vcom control
		   ILI9341_SendData   (0xbe);

		   //------------memory access control------------------------
		   ILI9341_SendCommand (ILI9341_MAC); // memory access control
		   ILI9341_SendData(0x48);

		   ILI9341_SendCommand (ILI9341_PIXEL_FORMAT); // pixel format set
		   ILI9341_SendData   (0x55); // 16bit /pixel

		   ILI9341_SendCommand(ILI9341_FRC);
		   ILI9341_SendData(0);
		   ILI9341_SendData(0x1F);
		   //-------------ddram ----------------------------
		   ILI9341_SendCommand (ILI9341_COLUMN_ADDR); // column set
		   ILI9341_SendData   (0x00); // x0_HIGH---0
		   ILI9341_SendData   (0x00); // x0_LOW----0
		   ILI9341_SendData   (0x00); // x1_HIGH---240
		   ILI9341_SendData   (0xEF); // x1_LOW----240
		   ILI9341_SendCommand (ILI9341_PAGE_ADDR); // page address set
		   ILI9341_SendData   (0x00); // y0_HIGH---0
		   ILI9341_SendData   (0x00); // y0_LOW----0
		   ILI9341_SendData   (0x01); // y1_HIGH---320
		   ILI9341_SendData   (0x3F); // y1_LOW----320

		   ILI9341_SendCommand (ILI9341_TEARING_OFF); // tearing effect off
		   //LCD_write_cmd(ILI9341_TEARING_ON); // tearing effect on
		   //LCD_write_cmd(ILI9341_DISPLAY_INVERSION); // display inversion
		   ILI9341_SendCommand (ILI9341_Entry_Mode_Set); // entry mode set
		   // Deep Standby Mode: OFF
		   // Set the output level of gate driver G1-G320: Normal display
		   // Low voltage detection: Disable
		   ILI9341_SendData   (0x07);
		   //-----------------display------------------------
		   ILI9341_SendCommand (ILI9341_DFC); // display function control
		   //Set the scan mode in non-display area
		   //Determine source/VCOM output in a non-display area in the partial display mode
		   ILI9341_SendData   (0x0a);
		   //Select whether the liquid crystal type is normally white type or normally black type
		   //Sets the direction of scan by the gate driver in the range determined by SCN and NL
		   //Select the shift direction of outputs from the source driver
		   //Sets the gate driver pin arrangement in combination with the GS bit to select the optimal scan mode for the module
		   //Specify the scan cycle interval of gate driver in non-display area when PTG to select interval scan
		   ILI9341_SendData   (0x82); // REV 1 normally white
		   //ILI9341_SendData   (0x02); // REV 0 normally black (colors inverted)
		   // Sets the number of lines to drive the LCD at an interval of 8 lines
		   ILI9341_SendData   (0x27);
		   ILI9341_SendData   (0x00); // clock divisor

		   ILI9341_SendCommand (ILI9341_SLEEP_OUT); // sleep out
		   HAL_Delay(100);
		   ILI9341_SendCommand (ILI9341_DISPLAY_ON); // display on
		   HAL_Delay(100);
		   ILI9341_SendCommand (ILI9341_GRAM); // memory write
		   HAL_Delay(5);
	 }


 }

//5. Write data to a single pixel
void ILI9341_DrawPixel(uint16_t x, uint16_t y, uint16_t color)
{
	ILI9341_SetCursorPosition(x, y, x, y);
	ILI9341_SendData(color>>8);
	ILI9341_SendData(color&0xFF);
}

void ILI9341_DrawPixel_uint16(uint16_t x, uint16_t y, uint16_t color)
{
	ILI9341_SetCursorPosition_uint16(x, y, x, y);
	ILI9341_SendData_uint16(color);
}

//6. Fill the entire screen with a background color
void ILI9341_Fill(uint16_t color)
{
	uint32_t n = ILI9341_PIXEL_COUNT;
	//uint16_t myColor = 0xFF;
	
	if(rotationNum==1 || rotationNum==3)
	{
		ILI9341_SetCursorPosition(0, 0,   ILI9341_WIDTH -1, ILI9341_HEIGHT -1);
	}
	else if(rotationNum==2 || rotationNum==4)
	{
		ILI9341_SetCursorPosition(0, 0, ILI9341_HEIGHT -1, ILI9341_WIDTH -1);
	}
	
	
	while (n)
	{
		n--;
		ILI9341_SendData(color>>8);
		ILI9341_SendData(color&0xff);
	}
}

// Fill the entire screen with a background color with DMA and needing a 512 byte buffer.
void ILI9341_Fill_DMA(uint16_t color, uint16_t w, uint16_t h, uint8_t* bufferColored)
{

	for (int i = 0; i < 512; i++)
	{
		bufferColored[i] = (color>>8);
		bufferColored[i+1] = (color&0xff);
		i++;
	}

	char isFirstBlock = 1;
	for(int i=0; i < 300; i++)
	{

		ILI9341_printImageSector_512bytes(0, 0, w, h, bufferColored, isFirstBlock);

		isFirstBlock = 0;

	}
}



void ILI9341_Fill_uint16(uint16_t color) {
	uint32_t n = ILI9341_PIXEL_COUNT;
	//uint16_t myColor = 0xFF;

	if(rotationNum==1 || rotationNum==3)
	{
		ILI9341_SetCursorPosition_uint16(0, 0,   ILI9341_WIDTH -1, ILI9341_HEIGHT -1);
	}
	else if(rotationNum==2 || rotationNum==4)
	{
		ILI9341_SetCursorPosition_uint16(0, 0, ILI9341_HEIGHT -1, ILI9341_WIDTH -1);
	}


	while (n)
	{
		n--;
		ILI9341_SendData_uint16(color);
	}
}
//7. Rectangle drawing functions
void ILI9341_Fill_Rect(unsigned int x0,unsigned int y0, unsigned int x1,unsigned int y1, uint16_t color) {
	uint32_t n = ((x1+1)-x0)*((y1+1)-y0);
	if (n>ILI9341_PIXEL_COUNT) n=ILI9341_PIXEL_COUNT;
	ILI9341_SetCursorPosition(x0, y0, x1, y1);
	while (n)
	{
		n--;
		ILI9341_SendData(color>>8);
		ILI9341_SendData(color&0xff);
	}
}

void ILI9341_Fill_Rect_uint16(unsigned int x0,unsigned int y0, unsigned int x1,unsigned int y1, uint16_t color) {
	uint32_t n = ((x1+1)-x0)*((y1+1)-y0);
	if (n>ILI9341_PIXEL_COUNT) n=ILI9341_PIXEL_COUNT;
		ILI9341_SetCursorPosition_uint16(x0, y0, x1, y1);
	while (n)
	{
		n--;
		ILI9341_SendData(color);
	}
}

//8. Circle drawing functions
void ILI9341_drawCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x = 0;
  int16_t y = r;

  ILI9341_DrawPixel(x0  , y0+r, color);
  ILI9341_DrawPixel(x0  , y0-r, color);
  ILI9341_DrawPixel(x0+r, y0  , color);
  ILI9341_DrawPixel(x0-r, y0  , color);

  while (x<y) {
    if (f >= 0) {
      y--;
      ddF_y += 2;
      f += ddF_y;
    }
    x++;
    ddF_x += 2;
    f += ddF_x;
  
    ILI9341_DrawPixel(x0 + x, y0 + y, color);
    ILI9341_DrawPixel(x0 - x, y0 + y, color);
    ILI9341_DrawPixel(x0 + x, y0 - y, color);
    ILI9341_DrawPixel(x0 - x, y0 - y, color);
    ILI9341_DrawPixel(x0 + y, y0 + x, color);
    ILI9341_DrawPixel(x0 - y, y0 + x, color);
    ILI9341_DrawPixel(x0 + y, y0 - x, color);
    ILI9341_DrawPixel(x0 - y, y0 - x, color);
  }
}
void drawCircleHelper( int16_t x0, int16_t y0, int16_t r, uint8_t cornername, uint16_t color)
{
	int16_t f     = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x     = 0;
  int16_t y     = r;

  while (x<y) {
    if (f >= 0) {
      y--;
      ddF_y += 2;
      f     += ddF_y;
    }
    x++;
    ddF_x += 2;
    f     += ddF_x;
    if (cornername & 0x4) {
      ILI9341_DrawPixel(x0 + x, y0 + y, color);
      ILI9341_DrawPixel(x0 + y, y0 + x, color);
    } 
    if (cornername & 0x2) {
      ILI9341_DrawPixel(x0 + x, y0 - y, color);
      ILI9341_DrawPixel(x0 + y, y0 - x, color);
    }
    if (cornername & 0x8) {
      ILI9341_DrawPixel(x0 - y, y0 + x, color);
      ILI9341_DrawPixel(x0 - x, y0 + y, color);
    }
    if (cornername & 0x1) {
      ILI9341_DrawPixel(x0 - y, y0 - x, color);
      ILI9341_DrawPixel(x0 - x, y0 - y, color);
    }
  }
}
void fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, uint16_t color)
{
	int16_t f     = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x     = 0;
  int16_t y     = r;

  while (x<y) {
    if (f >= 0) {
      y--;
      ddF_y += 2;
      f     += ddF_y;
    }
    x++;
    ddF_x += 2;
    f     += ddF_x;

    if (cornername & 0x1) {
      ILI9341_drawFastVLine(x0+x, y0-y, 2*y+1+delta, color);
      ILI9341_drawFastVLine(x0+y, y0-x, 2*x+1+delta, color);
    }
    if (cornername & 0x2) {
      ILI9341_drawFastVLine(x0-x, y0-y, 2*y+1+delta, color);
      ILI9341_drawFastVLine(x0-y, y0-x, 2*x+1+delta, color);
    }
  }
}
void ILI9341_fillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
	ILI9341_drawFastVLine(x0, y0-r, 2*r+1, color);
  fillCircleHelper(x0, y0, r, 3, 0, color);
}

//9. Line drawing functions
void ILI9341_drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color)
{
	int16_t steep = abs(y1 - y0) > abs(x1 - x0);
  if (steep) {
    swap(x0, y0);
    swap(x1, y1);
  }

  if (x0 > x1) {
    swap(x0, x1);
    swap(y0, y1);
  }

  int16_t dx, dy;
  dx = x1 - x0;
  dy = abs(y1 - y0);

  int16_t err = dx / 2;
  int16_t ystep;

  if (y0 < y1) {
    ystep = 1;
  } else {
    ystep = -1;
  }

  for (; x0<=x1; x0++) {
    if (steep) {
      ILI9341_DrawPixel(y0, x0, color);
    } else {
      ILI9341_DrawPixel(x0, y0, color);
    }
    err -= dy;
    if (err < 0) {
      y0 += ystep;
      err += dx;
    }
  }
}	

void ILI9341_drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
	ILI9341_drawLine(x, y, x+w-1, y, color);
}

void ILI9341_drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
	ILI9341_drawLine(x, y, x, y+h-1, color);
}
//10. Triangle drawing
void ILI9341_drawTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
	ILI9341_drawLine(x0, y0, x1, y1, color);
  ILI9341_drawLine(x1, y1, x2, y2, color);
  ILI9341_drawLine(x2, y2, x0, y0, color);
}
void ILI9341_fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
	int16_t a, b, y, last;

  // Sort coordinates by Y order (y2 >= y1 >= y0)
  if (y0 > y1) {
    swap(y0, y1); swap(x0, x1);
  }
  if (y1 > y2) {
    swap(y2, y1); swap(x2, x1);
  }
  if (y0 > y1) {
    swap(y0, y1); swap(x0, x1);
  }

  if(y0 == y2) { // Handle awkward all-on-same-line case as its own thing
    a = b = x0;
    if(x1 < a)      a = x1;
    else if(x1 > b) b = x1;
    if(x2 < a)      a = x2;
    else if(x2 > b) b = x2;
    ILI9341_drawFastHLine(a, y0, b-a+1, color);
    return;
  }

  int16_t
    dx01 = x1 - x0,
    dy01 = y1 - y0,
    dx02 = x2 - x0,
    dy02 = y2 - y0,
    dx12 = x2 - x1,
    dy12 = y2 - y1,
    sa   = 0,
    sb   = 0;

  // For upper part of triangle, find scanline crossings for segments
  // 0-1 and 0-2.  If y1=y2 (flat-bottomed triangle), the scanline y1
  // is included here (and second loop will be skipped, avoiding a /0
  // error there), otherwise scanline y1 is skipped here and handled
  // in the second loop...which also avoids a /0 error here if y0=y1
  // (flat-topped triangle).
  if(y1 == y2) last = y1;   // Include y1 scanline
  else         last = y1-1; // Skip it

  for(y=y0; y<=last; y++) {
    a   = x0 + sa / dy01;
    b   = x0 + sb / dy02;
    sa += dx01;
    sb += dx02;

    if(a > b) swap(a,b);
    ILI9341_drawFastHLine(a, y, b-a+1, color);
  }

  // For lower part of triangle, find scanline crossings for segments
  // 0-2 and 1-2.  This loop is skipped if y1=y2.
  sa = dx12 * (y - y1);
  sb = dx02 * (y - y0);
  for(; y<=y2; y++) {
    a   = x1 + sa / dy12;
    b   = x0 + sb / dy02;
    sa += dx12;
    sb += dx02;

    if(a > b) swap(a,b);
    ILI9341_drawFastHLine(a, y, b-a+1, color);
	}
}

//11. Text printing functions
void ILI9341_drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size)
{
	if(rotationNum == 1 || rotationNum ==3)
	{
		if((x >= ILI9341_WIDTH)            || // Clip right
     (y >= ILI9341_HEIGHT)           || // Clip bottom
     ((x + 6 * size - 1) < 0) || // Clip left
     ((y + 8 * size - 1) < 0))   // Clip top
    return;
	}
	else
	{
		if((y >= ILI9341_WIDTH)            || // Clip right
     (x >= ILI9341_HEIGHT)           || // Clip bottom
     ((y + 6 * size - 1) < 0) || // Clip left
     ((x + 8 * size - 1) < 0))   // Clip top
    return;
	}	
	

  if(!_cp437 && (c >= 176)) c++; // Handle 'classic' charset behavior

  for (int8_t i=0; i<6; i++ ) {
    uint8_t line;
    if (i == 5) 
      line = 0x0;
    else 
      line = pgm_read_byte(font1+(c*5)+i);
    for (int8_t j = 0; j<8; j++) {
      if (line & 0x1) {
        if (size == 1) // default size
          ILI9341_DrawPixel(x+i, y+j, color);
        else {  // big size
          ILI9341_Fill_Rect(x+(i*size), y+(j*size), size + x+(i*size), size+1 + y+(j*size), color);
        } 
      } else if (bg != color) {
        if (size == 1) // default size
          ILI9341_DrawPixel(x+i, y+j, bg);
        else {  // big size
          ILI9341_Fill_Rect(x+i*size, y+j*size, size + x+i*size, size+1 + y+j*size, bg);
        }
      }
      line >>= 1;
    }
  }
}

void ILI9341_drawChar_uint16(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size)
{
	if(rotationNum == 1 || rotationNum ==3)
	{
		if((x >= ILI9341_WIDTH)            || // Clip right
     (y >= ILI9341_HEIGHT)           || // Clip bottom
     ((x + 6 * size - 1) < 0) || // Clip left
     ((y + 8 * size - 1) < 0))   // Clip top
    return;
	}
	else
	{
		if((y >= ILI9341_WIDTH)            || // Clip right
     (x >= ILI9341_HEIGHT)           || // Clip bottom
     ((y + 6 * size - 1) < 0) || // Clip left
     ((x + 8 * size - 1) < 0))   // Clip top
    return;
	}


  if(!_cp437 && (c >= 176)) c++; // Handle 'classic' charset behavior

  for (int8_t i=0; i<6; i++ )
  {
    uint8_t line;
    if (i == 5)
      line = 0x0;
    else
      line = pgm_read_byte(font1+(c*5)+i);
    for (int8_t j = 0; j<8; j++)
    {
      if (line & 0x1)
      {
        if (size == 1) // default size
          ILI9341_DrawPixel_uint16(x+i, y+j, color);
        else
        {  // big size
          ILI9341_Fill_Rect_uint16(x+(i*size), y+(j*size), size + x+(i*size), size+1 + y+(j*size), color);
        }
      } else if (bg != color)
      {
        if (size == 1) // default size
          ILI9341_DrawPixel_uint16(x+i, y+j, bg);
        else
        {  // big size
          ILI9341_Fill_Rect_uint16(x+i*size, y+j*size, size + x+i*size, size+1 + y+j*size, bg);
        }
      }
      line >>= 1;
    }
  }
}

void ILI9341_printText(char text[], int16_t x, int16_t y, uint16_t color, uint16_t bg, uint8_t size)
{
	int16_t offset;
	offset = size*6;
	for(uint16_t i=0; i<40 && text[i]!=0; i++)
	{
		ILI9341_drawChar(x+(offset*i), y, text[i],color,bg,size);
	}
}

void ILI9341_printText_uint16(char text[], int16_t x, int16_t y, uint16_t color, uint16_t bg, uint8_t size)
{
	int16_t offset;
	offset = size*6;
	for(uint16_t i=0; i<40 && text[i]!=0; i++)
	{
		ILI9341_drawChar_uint16(x+(offset*i), y, text[i],color,bg,size);
	}
}

//12. Image print (RGB 565, 2 bytes per pixel)
void ILI9341_printImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *data, uint32_t size)
{
	uint32_t n = size;
	ILI9341_SetCursorPosition(x, y, w+x-1, h+y-1);
	for(uint32_t i=0; i<n ; i++)
	{
		ILI9341_SendData((data[i])>>8);
		ILI9341_SendData((data[i]) & 0xFF);
	}
}



void ILI9341_printImage_uint16(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *data)
{
	//uint32_t n = size;
	//uint32_t size = 76800;
	uint32_t size_half = 38400;
	ILI9341_SetCursorPosition(x, y, w+x-1, h+y-1);
	//uint32_t indice=0;
	//hspi2.Init.DataSize = SPI_DATASIZE_16BIT;

	/*while(n>16)
	{

		ILI9341_SendData_Multi(data[indice], 16);
		indice+=16;
		n-=16;
	}

	ILI9341_SendData_Multi(data[indice], n);
	*/
	__HAL_SPI_DISABLE(lcdSPIhandle);
	lcdSPIhandle->Init.DataSize = SPI_DATASIZE_16BIT;
	HAL_SPI_Init(lcdSPIhandle);

	/*for(uint32_t i=0; i<n ; i++)
	{
		ILI9341_SendData_uint16(data[i]);

	}
	*/


	//Set DC LOW for DATA mode
	HAL_GPIO_WritePin(tftDC_GPIO, tftDC_PIN, GPIO_PIN_SET);
	//Put CS LOW
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit_DMA(lcdSPIhandle, data, size_half);

	while(HAL_SPI_GetState(lcdSPIhandle) != HAL_SPI_STATE_READY){}

	HAL_SPI_Transmit_DMA(lcdSPIhandle, data + size_half, size_half);

	while(HAL_SPI_GetState(lcdSPIhandle) != HAL_SPI_STATE_READY){}


	//Bring CS HIGH
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);

	__HAL_SPI_DISABLE(lcdSPIhandle);
	lcdSPIhandle->Init.DataSize = SPI_DATASIZE_8BIT;
	HAL_SPI_Init(lcdSPIhandle);


}

//13. Set screen rotation
void ILI9341_setRotation(uint8_t rotate)
{
	switch(rotate)
	{
		case 1:
			rotationNum = 1;
			ILI9341_SendCommand(ILI9341_MEMCONTROL);
			ILI9341_SendData(ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR);
			break;
		case 2:
			rotationNum = 2;
			ILI9341_SendCommand(ILI9341_MEMCONTROL);
			ILI9341_SendData(ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR);
			break;
		case 3:
			rotationNum = 3;
			ILI9341_SendCommand(ILI9341_MEMCONTROL);
			ILI9341_SendData(ILI9341_MADCTL_MX | ILI9341_MADCTL_BGR);
			break;
		case 4:
			rotationNum = 4;
			ILI9341_SendCommand(ILI9341_MEMCONTROL);
			ILI9341_SendData(ILI9341_MADCTL_MX | ILI9341_MADCTL_MY | ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR);
			break;
		default:
			rotationNum = 1;
			ILI9341_SendCommand(ILI9341_MEMCONTROL);
			ILI9341_SendData(ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR);
			break;
	}
}

void ILI9341_setRotation_uint16(uint8_t rotate)
{
	switch(rotate)
	{
		case 1:
			rotationNum = 1;
			ILI9341_SendCommand_uint16(ILI9341_MEMCONTROL <<8);
			ILI9341_SendData_uint16((ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR) <<8);
			break;
		case 2:
			rotationNum = 2;
			ILI9341_SendCommand_uint16(ILI9341_MEMCONTROL <<8);
			ILI9341_SendData_uint16((ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR) <<8);
			break;
		case 3:
			rotationNum = 3;
			ILI9341_SendCommand_uint16(ILI9341_MEMCONTROL <<8);
			ILI9341_SendData_uint16((ILI9341_MADCTL_MX | ILI9341_MADCTL_BGR) <<8);
			break;
		case 4:
			rotationNum = 4;
			ILI9341_SendCommand_uint16(ILI9341_MEMCONTROL <<8);
			ILI9341_SendData_uint16((ILI9341_MADCTL_MX | ILI9341_MADCTL_MY | ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR) <<8);
			break;
		default:
			rotationNum = 1;
			ILI9341_SendCommand_uint16(ILI9341_MEMCONTROL <<8);
			ILI9341_SendData_uint16((ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR) <<8);
			break;
	}
}
//14. Get screen rotation
uint8_t ILI9341_getRotation(void)
{
	return rotationNum;
}

void ILI9341_inversion_on(void)
{
	ILI9341_SendCommand(ILI9341_INVERSION_ON);
}
void ILI9341_inversion_off(void)
{
	ILI9341_SendCommand(ILI9341_INVERSION_OFF);
}

void ILI9341_adjust_display_brightness(uint8_t brightness)
{
	ILI9341_SendCommand(ILI9341_WDB);
	ILI9341_SendData(brightness);

}

void ILI9341_control_display(uint8_t brightness_on, uint8_t displayDimming_on, uint8_t backlightControl_on)
{
	uint8_t control_register=0;

	if((brightness_on != 1) || (brightness_on != 0))
		return;
	if((displayDimming_on != 1) || (displayDimming_on != 0))
		return;
	if((backlightControl_on != 1) || (backlightControl_on != 0))
		return;

	control_register += (brightness_on <<5) + (displayDimming_on <<3) + (backlightControl_on <<2);
	ILI9341_SendCommand(ILI9341_WCD);
	ILI9341_SendData(control_register);
}

void ILI9341_printImageSector_512bytes(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t* bufferRx, bool isFirstBlock)
{
	if(isFirstBlock)
	{
		ILI9341_SetCursorPosition(x, y, w+x-1, h+y-1);
	}
	ILI9341_BSP_DCset_dataMode();
	ILI9341_BSP_select_CS();
	ILI9341_BSP_SPI_transmit(bufferRx, 512);
	ILI9341_BSP_SPI_transmit_DMA_waitReady();
	ILI9341_BSP_deselect_CS();
}

/*
 * needs a 512 byte-buffer to send.
 * will start transfer via DMA and wait until it finishes to end the transfer.
 */
void ILI9341_printImageSector_512bytes_DMA_start_finish(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t* bufferRx, bool isFirstBlock)
{
	if(isFirstBlock)
	{
		ILI9341_SetCursorPosition(x, y, w+x-1, h+y-1);
	}
	ILI9341_BSP_DCset_dataMode();
	ILI9341_BSP_select_CS();
	ILI9341_BSP_SPI_transmit_DMA_start(bufferRx, 512);
	ILI9341_BSP_SPI_transmit_DMA_waitReady();
	ILI9341_BSP_SPI_transmit_DMA_finish();
}

/*
 * needs a 512 byte-buffer to send.
 * will start transfer via DMA but doesn't wait until it finishes.
 * user needs to call ILI9341_BSP_SPI_transmit_DMA_finish() once it's finished.
 */
void ILI9341_printImageSector_512bytes_DMA_start(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t* bufferRx, bool isFirstBlock)
{
	if(isFirstBlock)
	{
		ILI9341_SetCursorPosition(x, y, w+x-1, h+y-1);
	}
	ILI9341_BSP_DCset_dataMode();
	ILI9341_BSP_select_CS();
	ILI9341_BSP_SPI_transmit_DMA_start(bufferRx, 512);
}

/*
 * Must be called when the DMA transfer of the printImageSector_512 finishes.
 */
void ILI9341_printImageSector_512bytes_DMA_finish(void)
{
	ILI9341_BSP_SPI_transmit_DMA_finish();
}


/**
  * @brief  inicializa lo necesario para el funcionamiento del ILI9341 tft
  *
  * @note   el módulo está pensado para funcionar con SPI modo 8 bits.
  *
  * @param  self:  puntero al ILI9341_t que el usuario tendrá que crear.
  *
  * @param  variables: puntero a la estructura con todos los campos de HW que necesitará.
  *         El usuario tendrá que crear e inicializar una para inyectarla.
  *
  * @retval None
  */
void ILI9341_Class_Init(ILI9341_t* self, ILI9341_BSP_variables_t* variables)
{
	// init HW:
	ILI9341_BSP_init(variables);

	// init functions:
	self->fillRectangle = ILI9341_Fill_Rect;
	self->drawPixel = ILI9341_DrawPixel;
	self->fill = ILI9341_Fill;
	self->fill_DMA = ILI9341_Fill_DMA;
	self->drawCircumference = ILI9341_drawCircle;
	self->drawLine = ILI9341_drawLine;
	self->fillCircle = ILI9341_fillCircle;
	self->drawFastHLine = ILI9341_drawFastHLine;
	self->drawFastVLine = ILI9341_drawFastVLine;
	self->drawTriangleOutline = ILI9341_drawTriangle;
	self->fillTriangle = ILI9341_fillTriangle;
	self->drawChar = ILI9341_drawChar;
	self->printText = ILI9341_printText;
	self->printImage = ILI9341_printImage;
	self->setRotation = ILI9341_setRotation;
	self->getRotation = ILI9341_getRotation;
	self->inversionON = ILI9341_inversion_on;
	self->inversionOFF = ILI9341_inversion_off;
	self->adjustDisplayBrightness = ILI9341_adjust_display_brightness;
	self->controlDisplay = ILI9341_control_display;
	self->printImageSector_512bytes = ILI9341_printImageSector_512bytes;
	self->printImageSector_512bytes_DMA_start_finish = ILI9341_printImageSector_512bytes_DMA_start_finish;
	self->printImageSector_512bytes_DMA_start = ILI9341_printImageSector_512bytes_DMA_start;
	self->printImageSector_512bytes_DMA_finish = ILI9341_printImageSector_512bytes_DMA_finish;

	// init pantalla:
	ILI9341_BSP_deselect_CS();
	//RESET pin -> turn LCD ON:
	ILI9341_BSP_reset();
	ILI9341_SendCommand (ILI9341_RESET); // software reset comand
	ILI9341_BSP_ms_delay(100);

	ILI9341_SendCommand (ILI9341_DISPLAY_OFF); // display off
	//------------power control------------------------------
	ILI9341_SendCommand (ILI9341_POWER1); // power control
	ILI9341_SendData   (0x26); // GVDD = 4.75v
	ILI9341_SendCommand (ILI9341_POWER2); // power control
	ILI9341_SendData   (0x11); // AVDD=VCIx2, VGH=VCIx7, VGL=-VCIx3
	//--------------VCOM-------------------------------------
	ILI9341_SendCommand (ILI9341_VCOM1); // vcom control
	ILI9341_SendData   (0x35); // Set the VCOMH voltage (0x35 = 4.025v)
	ILI9341_SendData   (0x3e); // Set the VCOML voltage (0x3E = -0.950v)
	ILI9341_SendCommand (ILI9341_VCOM2); // vcom control
	ILI9341_SendData   (0xbe);

	//------------memory access control------------------------
	ILI9341_SendCommand (ILI9341_MAC); // memory access control
	ILI9341_SendData(0x48);

	ILI9341_SendCommand (ILI9341_PIXEL_FORMAT); // pixel format set
	ILI9341_SendData   (0x55); // 16bit /pixel

	ILI9341_SendCommand(ILI9341_FRC);
	ILI9341_SendData(0);
	ILI9341_SendData(0x1F);
	//-------------ddram ----------------------------
	ILI9341_SendCommand (ILI9341_COLUMN_ADDR); // column set
	ILI9341_SendData   (0x00); // x0_HIGH---0
	ILI9341_SendData   (0x00); // x0_LOW----0
	ILI9341_SendData   (0x00); // x1_HIGH---240
	ILI9341_SendData   (0xEF); // x1_LOW----240
	ILI9341_SendCommand (ILI9341_PAGE_ADDR); // page address set
	ILI9341_SendData   (0x00); // y0_HIGH---0
	ILI9341_SendData   (0x00); // y0_LOW----0
	ILI9341_SendData   (0x01); // y1_HIGH---320
	ILI9341_SendData   (0x3F); // y1_LOW----320

	ILI9341_SendCommand (ILI9341_TEARING_OFF); // tearing effect off
	//LCD_write_cmd(ILI9341_TEARING_ON); // tearing effect on
	//LCD_write_cmd(ILI9341_DISPLAY_INVERSION); // display inversion
	ILI9341_SendCommand (ILI9341_Entry_Mode_Set); // entry mode set
	// Deep Standby Mode: OFF
	// Set the output level of gate driver G1-G320: Normal display
	// Low voltage detection: Disable
	ILI9341_SendData   (0x07);
	//-----------------display------------------------
	ILI9341_SendCommand (ILI9341_DFC); // display function control
	//Set the scan mode in non-display area
	//Determine source/VCOM output in a non-display area in the partial display mode
	ILI9341_SendData   (0x0a);
	//Select whether the liquid crystal type is normally white type or normally black type
	//Sets the direction of scan by the gate driver in the range determined by SCN and NL
	//Select the shift direction of outputs from the source driver
	//Sets the gate driver pin arrangement in combination with the GS bit to select the optimal scan mode for the module
	//Specify the scan cycle interval of gate driver in non-display area when PTG to select interval scan
	ILI9341_SendData   (0x82); // REV 1 normally white
	//ILI9341_SendData   (0x02); // REV 0 normally black (colors inverted)
	// Sets the number of lines to drive the LCD at an interval of 8 lines
	ILI9341_SendData   (0x27);
	ILI9341_SendData   (0x00); // clock divisor

	ILI9341_SendCommand (ILI9341_SLEEP_OUT); // sleep out
	ILI9341_BSP_ms_delay(100);
	ILI9341_SendCommand (ILI9341_DISPLAY_ON); // display on
	ILI9341_BSP_ms_delay(100);
	ILI9341_SendCommand (ILI9341_GRAM); // memory write
	ILI9341_BSP_ms_delay(5);
}

